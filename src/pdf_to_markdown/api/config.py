...
class Config:
    """Configuration container for pdf-to-markdown."""
        ...
        if 'llm_provider' in self._config:
            ...
            if provider['provider_type'] == 'openai':
                ...
                if 'model_name' not in provider:
                    raise ConfigurationError("llm_provider.model_name is required for Transformers provider")
...
class ConfigBuilder:
    ...
    def with_openai(self, api_key: str, model: str = "gpt-4o-mini", 
                     endpoint: str = "https://api.openai.com/v1", **kwargs) -> 'ConfigBuilder':
    ...
    def with_transformers(self, model_name: str, device: str = "auto", **kwargs) -> 'ConfigBuilder':
        ...
        return self.with_llm_provider(
            ...
            max_tokens=kwargs.get('max_tokens', 4096),
            temperature=kwargs.get('temperature', 0.1),
            **{k: v for k, v in kwargs.items() if k not in ['torch_dtype', 'max_tokens', 'temperature']}
        )
    ...
    def with_cache_dir(self, cache_dir: Union[str, Path]) -> 'ConfigBuilder':
    ...
    def with_validation_correction(self, enabled: bool = True, max_attempts: int = 2) -> 'ConfigBuilder':
