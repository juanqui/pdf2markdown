...
class RepetitionValidator(BaseValidator):
                ...
                if consecutive_count == self.consecutive_threshold:
                    ...
                    issues.append(
                        ValidationIssue(
                            ...
                            extra_info=f'Content: "{line[:50]}..."' if len(line) > 50 else f'Content: "{line}"',
                            severity="error",
                        )
                    )
                    ...
                    for issue in issues:
                        if issue.rule_id == "REP001" and issue.extra_info and line[:50] in issue.extra_info:
                            issue.description = f"Line repeated {consecutive_count} times consecutively"
                            break
            else:
                consecutive_count = 1
                last_line = line
    ...
    def _check_paragraph_duplicates(self, content: str) -> list[ValidationIssue]:
        ...
        for para, line_numbers in paragraph_map.items():
        ...
        for rule_name, rule_issues in issues_by_type.items():
            if rule_name == "consecutive-duplicates":
                ...
                for issue in rule_issues[:3]:
                    ...
                    if issue.extra_info:
                        instructions += f"  {issue.extra_info}\n"
                instructions += "**Fix**: Remove duplicate occurrences, keeping only unique content.\n\n"
