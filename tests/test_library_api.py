...
class TestConfigBuilder:
    ...
    def test_with_openai(self):
        ...
        config = ConfigBuilder() \
            .with_openai(api_key="test-key", model="gpt-4o") \
            .build()
    ...
    def test_with_transformers(self):
        ...
        config = ConfigBuilder() \
            .with_transformers(model_name="test-model", device="cpu") \
            .build()
...
class TestPDFConverter:
    ...
    def mock_converter(self):
        ...
        with patch('pdf_to_markdown.api.converter.create_llm_provider') as mock_create_llm, \
             patch('pdf_to_markdown.api.converter.SimpleDocumentParser') as mock_doc_parser, \
             patch('pdf_to_markdown.api.converter.SimpleLLMPageParser') as mock_page_parser, \
             patch('pdf_to_markdown.api.converter.PipelineCoordinator') as mock_pipeline:
    ...
    async def test_convert_async(self, tmp_path):
        ...
        with patch('pdf_to_markdown.api.converter.create_llm_provider') as mock_create_llm, \
            ...
            from pdf_to_markdown.core.models import Document, Page, ProcessingStatus
    ...
    async def test_stream_pages(self, tmp_path):
        ...
        with patch.object(converter, '_ensure_initialized'), \
             patch.object(converter, '_pipeline') as mock_pipeline:
            
            # Mock pipeline methods
            mock_pipeline.start_processing = AsyncMock(return_value="doc-id")
            mock_pipeline.get_status = AsyncMock(side_effect=[
                {'status': 'processing', 'total_pages': 2, 'pages': {1: 'completed'}},
                {'status': 'completed', 'total_pages': 2, 'pages': {1: 'completed', 2: 'completed'}}
            ])
            
            from pdf_to_markdown.core.models import Page, ProcessingStatus
            mock_page1 = Page(
                id="page1", document_id="doc-id", page_number=1,
                markdown_content="Page 1", status=ProcessingStatus.COMPLETED
            )
            mock_page2 = Page(
                id="page2", document_id="doc-id", page_number=2,
                markdown_content="Page 2", status=ProcessingStatus.COMPLETED
            )
